import{c as f,q as R,F as C,s as T,k as L,i as q,a as k}from"../../footer-CdoaRbOv.js";import{l as U}from"../../lazy-loading-Cecxv2lM.js";import{i as w}from"../../breadcrumbs-BpSfuU0S.js";import{a as F}from"../../accordion-BLR9faGd.js";import{D as y}from"../../dropdown-B85-OCRO.js";import{c as A,r as M,s as g}from"../../index-B-aVQfpU.js";function I(o){return f(o,{tag:"div",render(t,d,l,{runOnce:n}){const{items:i=[],userId:r}=d;n&&(t.className="cart-table",t.innerHTML=`
          ...
          `,t._els={list:t.querySelector(".cart-table__list")}),O(t._els.list,i,r)}})}function O(o,t,d){o.innerHTML="",!(!t||!Array.isArray(t))&&t.forEach(l=>{const n=R({product:l,userId:d});o.appendChild(n)})}function P(o){return f(o,{tag:"div",render(t,d,l,{runOnce:n}){if(n){t.className="order-summary",t.innerHTML=`
        ...
        `;let i="",r="";const c=120,x=t.querySelector(".order-summary__block_shipping-estimate"),E=F({items:[{title:"Estimate Shipping and Tax",content:j(),isActive:!0}],isSingleOpen:!0});x.append(E);const b=y({name:"country",placeholder:"Select country",options:A}),u=y({name:"state",placeholder:"Select state",options:[],disabled:!0});t.querySelector(".shipping-estimate__control_country").append(b),t.querySelector(".shipping-estimate__control_state").append(u);const S=()=>{const e=t.querySelectorAll(".js-summary-btn"),s=i!==""&&r!=="";e.forEach(a=>{a.disabled=!s,a.classList.toggle("button_disabled",!s)})},_=e=>{const s=t.querySelector(".js-tax-value"),a=t.querySelector(".js-total-value");if(!e){s.textContent="0.00 EUR",a.textContent=`${c.toFixed(2)} EUR`;return}const h=t.querySelector('input[name="shipping"]:checked')?.id==="shipping-flat"?e.flatRate:e.bestWay;s.textContent=`${h.toFixed(2)} EUR`,a.textContent=`${(c+h).toFixed(2)} EUR`},v=e=>{const s=t.querySelector('label[for="shipping-flat"]'),a=t.querySelector('label[for="shipping-best"]');e?(s.textContent=`Fixed ${e.flatRate.toFixed(2)} EUR`,a.textContent=`Table Rate ${e.bestWay.toFixed(2)} EUR`):(s.textContent="Fixed 0.00 EUR",a.textContent="Table Rate 0.00 EUR")},m=(e=!1)=>{const s=t.querySelectorAll('input[name="shipping"]'),a=g[i]?.[r];i&&r&&a?(s.forEach(p=>p.disabled=!1),e&&(s[0].checked=!0),v(a),_(a)):(s.forEach(p=>{p.disabled=!0,p.checked=!1}),v(null),_(null)),S()};b.addEventListener("onChange",e=>{i=e.detail,r="";const s=M[i]||[];u.update({options:s,disabled:s.length===0,defaultValue:""}),m()}),u.addEventListener("onChange",e=>{r=e.detail,m(!0)}),t.addEventListener("change",e=>{if(e.target.name==="shipping"){const s=g[i]?.[r];_(s)}}),m()}}})}function j(){return`
  ...
  `}const D=async()=>{w(".cart-section__breadcrumbs");const o=document.querySelector(".cart-section__col-1"),t=document.querySelector(".cart-section__col-2"),l=(await T.auth.getUser())?.data?.user?.id||null,n=I({items:[],userId:l}),i=P();o.append(n),t.append(i),L.subscribe("cart",async r=>{const c=r.viewItems;console.log("cartpage: cartViewsItems",c),n.update({items:c})})};document.addEventListener("DOMContentLoaded",async()=>{q(),D(),k(),U(".lazy",{rootMargin:"200px 0px"})});
