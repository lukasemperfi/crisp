import{c as L,b as E}from"./footer-D6qklo8q.js";function w(a){return L(a,{tag:"div",render(e,t,i){const{colors:s=[],title:d="Color",showTitle:l=!0,maxVisibleColors:n=null,selectionMode:c="multiple",selectedId:h=null}=t;if(!Array.isArray(s)){console.error("ColorFilter Error: colors array not found.");return}const _=typeof n=="number"&&n>0,p=_?s.slice(0,n):s,y=_?s.slice(n):[],g=p.map(r=>M(r,!1,h)).join(""),b=y.map(r=>M(r,!0,h)).join(""),u=_&&y.length?'<button class="color-filter__more-btn" type="button">…</button>':"";if(e.className="color-filter",e.innerHTML=`
      ...
      `,_&&y.length){const r=e.querySelector(".color-filter__more-btn"),m=e.querySelectorAll(".color-filter__item_is-hidden");r&&r.addEventListener("click",()=>{m.forEach(f=>{f.style.display=""}),r.remove()})}const o=e.querySelectorAll(".color-filter__input");c==="single"&&h!=null&&o.forEach(r=>{r.checked=Number(r.value)===Number(h)}),c==="single"?o.forEach(r=>{r.addEventListener("change",()=>{r.checked&&(o.forEach(m=>{m!==r&&(m.checked=!1)}),i("onChange",{selected:Number(r.value)}))})}):o.forEach(r=>{r.addEventListener("change",()=>{const m=Array.from(o).filter(f=>f.checked).map(f=>Number(f.value));i("onChange",{selected:m})})})}})}function M(a,e=!1,t=null){const i=(a.hex_code||"").toLowerCase(),d=i==="#ffffff"||i==="#fff"||i==="white"?"color-filter__box_is-white":"",l=e?"color-filter__item_is-hidden":"",n=Number(a.id)===Number(t);return`
    <label
